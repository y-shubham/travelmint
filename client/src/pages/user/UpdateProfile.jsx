...
const UpdateProfile = () => {
  ...
  const profileErrors = useMemo(() => {
    ...
    // Address optional here (kept for compatibility across app)
    return e;
  }, [formData]);
  ...
  const handleProfileSubmit = async (e) => {
    ...
    try {
      ...
      if (data.success && (res.status === 201 || res.status === 200)) {
        ...
        dispatch(updateUserSuccess()); // keep store consistent
        ...
        {tab === "profile" ? (
          <form className="space-y-4" onSubmit={handleProfileSubmit} noValidate>
            ...
            <div className="space-y-1">
              ...
              <input
                id="address" // keep id to preserve payload shape
                type="text"
                value={formData.address}
                onChange={(e) => {
                  setFormData((p) => ({ ...p, address: e.target.value })); // still stored in 'address'
                  setMsg({ type: "", text: "" });
                }}
                onBlur={onBlur}
